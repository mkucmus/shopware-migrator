...
const getProducts = async () =>
  await request.get(
    "http://es.vuestorefront.io:9200/vue_storefront_shopware/product/_search?size=60"
  );
let i = 0;
getProducts().then(async products => {
  ...
  for (const product of fakeProducts.data.hits.hits) {
    ...
    ++i;
    try {
      const visibilitiesResponse = await getAdmin(
        `product/${productId}/visibilities`,
        i % 5 == 0
      );
      ...
      for (const visibility of visibilities) {
        await deleteAdmin(
          `product/${productId}/visibilities/${visibility.id}`,
          i % 5 == 0
        );
      ...
      await patchAdmin(
        `product/${productId}?_response=true`,
        {
          visibilities: [
            {
