...
const getProducts = (from = 1, size = 100) => {
  return request.get(
    `https://fashion.storefrontcloud.io/api/catalog/vue_storefront_catalog/product/_search?q=type_id:configurable&size=${size}&from=${from}`
  );
};
const deleteProduct = id =>
  request.delete(
    `http://elasticsearch:9200/vue_storefront_catalog/product/${id}`
  );
const buildImgUrl = filename =>
  `https://magento-demo3.storefrontcloud.io/media/catalog/product${filename}`;
